...
void LCD(void*p)
{
    ...
    u0_dbg_printf("\nInside ");
    if (cam.begin()) {
       u0_dbg_printf("Camera Found:");
     } else {
       u0_dbg_printf("No camera found?");
       return;
     }
     // Print out the camera version information (optional)
     char *reply = cam.getVersion();
     if (reply == 0) {
       u0_dbg_printf("Failed to get version");
     } else {
       u0_dbg_printf("-----------------");
       u0_dbg_printf(reply);
       u0_dbg_printf("-----------------");
...
int main(void)
{
    ...
    xTaskCreate(LCD,"LCD",1024,(void*)1,tskIDLE_PRIORITY, NULL);
   // xTaskCreate(watchdog_tsk, "watchdog_task", 1024 * 4, (void*) 1, PRIORITY_HIGH, NULL);

    //----LAB2----
    // Optional Homework 1 : Transformation Pipeline and Perspective Projection
