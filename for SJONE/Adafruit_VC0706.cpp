...
bool Adafruit_VC0706::begin(uint16_t baud) {
    Uart3::getInstance().setBaudRate(baud);
    Uart2::getInstance().setBaudRate(baud);
  return reset();
}
...
void Adafruit_VC0706::sendCommand(uint8_t cmd, uint8_t args[] = 0, uint8_t argn = 0) {
    //put chars
    Uart3::getInstance().putChar(0x56, 0);
    Uart3::getInstance().putChar(serialNum, 0);
    Uart3::getInstance().putChar(cmd, 0);

    for (uint8_t i = 0; i < argn; i++) {
        Uart3::getInstance().putChar(args[i], 0);
    }

    Uart2::getInstance().putChar(0x56, 0);
    Uart2::getInstance().putChar(serialNum, 0);
    Uart2::getInstance().putChar(cmd, 0);

    for (uint8_t i=0; i<argn; i++) {
      Uart2::getInstance().putChar(args[i], 0);
      //Serial.print(" 0x");
...
uint8_t Adafruit_VC0706::readResponse(uint8_t numbytes, uint8_t timeout) {
  ...
  while ((timeout != counter) && (bufferLen != numbytes)){
    ...
    avail = Uart2::getInstance().getRxQueueSize();
    ...
    char c = camerabuff[bufferLen++];
    Uart2::getInstance().getChar(&c,0);
