...
//            u3.putChar(0x99);
        ...
        //getVersion
        u0_dbg_printf("SENT setimagesize\n");
...
void snapshot(void *p)
{
    ...
    while (jpglen > 0) {
       ...
       // uint8_t *buff;
        ...
        u3.putChar(0x00);
        u3.putChar(0x00);
        u3.putChar(frameptr >> 8);
        u3.putChar(frameptr & 0xFF);
        u3.putChar(0x00);
        u3.putChar(0x00);
        u3.putChar(0x00);
        u3.putChar(bytesToRead);
        u3.putChar(CAMERADELAY >> 8);
        u3.putChar(CAMERADELAY & 0xFF);
        rx_func();

        num_bytes = bytesToRead + 5;
       // rx_func();

        //writing
        //data = buffer.str();
       // char line[100] = {0};
       //u0_dbg_printf("buffer=>\n%s\n",buffer);
       // FRESULT f = Storage::append("1:c.txt", buffer, bytesToRead, 0);
...
//    xTaskCreate(tx_tsk, "TX", 1024 * 4, (void*) 1, PRIORITY_MEDIUM, NULL);
    xTaskCreate(snapshot, "snap", 1024 * 4, (void*) 1, PRIORITY_MEDIUM, NULL);
